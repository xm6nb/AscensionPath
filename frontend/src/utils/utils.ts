...
// 随机jpg展示图片
export function RandomJpgImg(seed: number) {
  const Count = 10 // 本地头像图片数量
  const randomIndex = (parseInt(String(seed)) % Count) + 1
  return new URL(`../assets/img/avatar/avatar${randomIndex}.jpg`, import.meta.url).href
}

// 随机png展示图片
export function RandomPngImg(seed: number) {
  const Count = 10 // 本地头像图片数量
  const randomIndex = (parseInt(String(seed)) % Count) + 1
  return new URL(`../assets/img/cover/image${randomIndex}.png`, import.meta.url).href
}