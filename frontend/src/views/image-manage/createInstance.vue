<template>
  <div
    class="page-content article-list custom-loading-svg"
    v-loading="loading"
  ...
  >
    ...
    >
      <div class="offset">
        <div
          class="item"
          v-for="(item, index) in vulImageList"
    ...
    <el-dialog v-model="dialogTableVisible" title="场景信息" width="800">
      <el-descriptions
        class="margin-top custom-loading-svg"
        title=""
        size="default"
        direction="vertical"
        border
      ...
      >
        <template #extra style="display: flex">
          <el-button type="info" v-if="useUserStore().getUserInfo.role==='admin'" color="#696969" @click="removeVulEnv(vulInfo.vulEnvID)">删除该漏洞环境</el-button>
          <el-button type="primary" @click="createInstance" v-if="vulInfo.status !== 1"
            >创建场景实例</el-button
          >
          <el-button type="warning" @click="extendTime(vulInfo.id)" v-if="vulInfo.status == 1"
            >延长实例时间</el-button
          >
          <el-button type="danger" @click="removeInstance" v-if="vulInfo.status == 1"
            >移除场景实例</el-button
          >
        </template>
        ...
        <el-descriptions-item>
          ...
          <template
            v-if="vulInfo.degree && vulInfo.degree.HoleType && vulInfo.degree.HoleType.length > 0"
          >
...
<script setup lang="ts">
  ...
  // 移除漏洞环境
  const removeVulEnv = async (id: number) => {
    createloading.value = true
    await api
      .post<BaseResult>({
